---
layout: post
title:  vera 3.0.0 api
category: api
description: vera 3.0.0版本的api文档
---

## 写在前面

### 命名规则

为了和相近的开源产品命名保持一致或者相近，vera 库样式或者变量命名规则参考 bootstrap 和 ionic 。

### 使用方法

vera 依赖 zepto.js 或者 jquery，就像 bootstrap 依赖 jquery 一样，在引入 vera.js前，请务必先引入 zepto.js 或者 jquery。建议是用 jquery 3.0.0 。 如果想屏蔽 click 300ms 延迟，根据需要引入 fastclick.js 。

具体code

	<script type="text/javascript" src="path/jquery.3.0.0.js"></script>
	<script type="text/javascript" src="path/vera.js"></script>

## 字体 font

目前库支持的字体都列在下面的页面，根据开发迭代进行，会持续增加字体库。说明，UI 提供的 svg 必须是 AI 导出，Sketch 出的图无法正常打包到字体库。图标的 svg 标签务必只能有 path g 基础标签，不能含有其他高级图像标签。

效果如下

[字体 font](http://wikieswan.github.io/vera3/example/index.html#font)

<iframe src="http://wikieswan.github.io/vera3/example/index.html#font"
 style="width:375px; height: 667px;width:375px; height: 667px;border: 2px solid #ddd;border-bottom: 50px solid #6D6767;border-top: 50px solid #6D6767;border-radius: 10px;"></iframe>



## 布局 layout

效果如下

[布局 layout](http://wikieswan.github.io/vera3/example/index.html#layout)

<iframe src="http://wikieswan.github.io/vera3/example/index.html#layout"
 style="width:375px; height: 667px;width:375px; height: 667px;border: 2px solid #ddd;border-bottom: 50px solid #6D6767;border-top: 50px solid #6D6767;border-radius: 10px;"></iframe>

布局使用栅格系统，和bootstrap相似，不过vera 采用的是 10 列结构 。语法结构如下

	<div class="container">
		<div class="row">
			<div class="col"></div>
		</div>
	</div>

页面所有内容，都报过在 	```container``` 下面。 ```row``` 表示行。行距默认是 0 ，如果想控制行距，包括行与行之间的距离、行与页面左右两侧的距离，可以通过下面的样式类来控制。

### 行与页面左右两侧的距离

	.padding-h-10 //行到页面左右两侧间距 10px
	.padding-h-15
	.padding-h-20
	.padding-h-25
	.padding-h-30
	.padding-h-left-null //行到页面左侧间距 0
	.padding-h-right-null//行到页面右侧间距 0



### 行与行之间的距离
	
	.margin-v-10	//行的上下margin是 10px
	.margin-v-15
	.margin-v-20
	.margin-v-25
	.margin-v-30
	.margin-v-30
	.margin-v-40
	.margin-v-50
	.margin-v-top-null		//行的上margin是 0
	.margin-v-bottom-null	//行的下margin是 0


### 列 col

	.col 		//列的宽度占整行的 100%
	.col-10		//列的宽度占整行的 10%
	.col-20
	.col-25
	.col-30
	.col-33
	.col-40
	.col-50
	.col-60
	.col-67
	.col-70
	.col-75
	.col-80
	.col-90


正常情况下，这种布局风格可以满足大部分页面布局。


## 按钮 button

效果如下

[按钮 button](http://wikieswan.github.io/vera3/example/index.html#button)

<iframe src="http://wikieswan.github.io/vera3/example/index.html#button"
 style="width:375px; height: 667px;border: 2px solid #ddd;border-bottom: 50px solid #6D6767;border-top: 50px solid #6D6767;border-radius: 10px;"></iframe>

### 用法

	//普通短按钮
	<button class="btn btn-basic">按钮</button> 
	<button class="btn btn-basic" disabled>按钮</button>

	//红色边框按钮
	<button class="btn btn-border">按钮</button>
	<button class="btn btn-border" disabled>按钮</button>

	//灰色边框按钮
	<button class="btn btn-border btn-border-grey">按钮</button>
	<button class="btn btn-border btn-border-grey" disabled>按钮</button>

	//红色边框按钮的三个不同尺寸
	<button class="btn btn-border">按钮</button>
	<button class="btn btn-border btn-small">按钮</button>
	<button class="btn btn-border btn-mini">按钮</button>

	//红色整行按钮
	<button class="btn btn-basic btn-full">按钮</button>
	<button class="btn btn-basic btn-full" disabled>按钮</button>

### 方法

	/***
		param: {
			target: '#myBtnId' // '.myBtnId'
		}
		return bool,返回目标 button 是否为 disabled 状态，true 是 disabled 状态
	*/
	getButtonStatus(param)//获取按钮是否为 disabled 状态


	/***
		param: {
			target: '#myBtnId', // '.myBtnId'
			disabled: false / true
		}
		return void
	*/
	setButtonStatus(param)//设置按钮的状态

### 用例

	$('#J-btn-switch').click(function(){
		var isdisabled  = vera.getButtonStatus({target:'.J-btn-status'})
		if(isdisabled){
			vera.setButtonStatus({
				target: '.J-btn-status',
				disabled: false
			})
		}
		else{
			vera.setButtonStatus({
				target: '.J-btn-status',
				disabled: true
			})
		}
	})

## 标签 label
	
效果如下

[标签 label](http://wikieswan.github.io/vera3/example/index.html#label)

<iframe src="http://wikieswan.github.io/vera3/example/index.html#label"
 style="width:375px; height: 667px;border: 2px solid #ddd;border-bottom: 50px solid #6D6767;border-top: 50px solid #6D6767;border-radius: 10px;"></iframe>

### 用法

	<p class="demo-p">标签 </p>
	<div class="row margin-v-10">
		<div class="col">
			<div class="label-wrap">
				<div class="label-title">
					选择银行
				</div>
				<div class="label-content">
					招商银行
				</div>
				<div class="label-icon">
					<i class="icon-arrow"></i>
				</div>
			</div>
		</div>
	</div>
	<div class="row margin-v-10">
		<div class="col">
			<div class="label-wrap">
				<div class="label-title">
					银行
				</div>
				<div class="label-tip">
					点击选择银行
				</div>
				<div class="label-icon">
					<i class="icon-arrow"></i>
				</div>
			</div>
		</div>
	</div>


## 表单 input

效果如下

[表单 input](http://wikieswan.github.io/vera3/example/index.html#input)

<iframe src="http://wikieswan.github.io/vera3/example/index.html#input"
 style="width:375px; height: 667px;border: 2px solid #ddd;border-bottom: 50px solid #6D6767;border-top: 50px solid #6D6767;border-radius: 10px;"></iframe>

### 用法

	<p class="demo-p">1 标准表单 </p>
	<div class="row margin-v-10">
		<div class="col">
			<div class="input-wrap">
				<div class="input-label">文本表单</div>
				<div class="input-holder">
					<input type="text" value="Sample Text" />
				</div>
				<div class="input-del" >
					<i class="icon-delet" role="button"></i>
				</div>
			</div>
		</div>
	</div>
		

	<div class="row margin-v-10">
		<div class="col">
			<div class="input-wrap">
				<div class="input-label">手机号</div>
				<div class="input-holder">
					<input type="tel" formatType="mobile" placeholder="请输入手机号">
				</div>
				<div class="input-del">
					<i class="icon-delet" role="button"></i>
				</div>
			</div>
		</div>
	</div>

	<div class="row margin-v-10">
		<div class="col">
			<div class="input-wrap">
				<div class="input-label">银行卡号</div>
				<div class="input-holder">
					<input type="tel" formatType="bankCard" placeholder="请输入银行卡号">
				</div>
				<div class="input-del">
					<i class="icon-delet" role="button"></i>
				</div>
				<div class="input-icon">
					<i class="icon-card" role="button"></i>
				</div>
			</div>
		</div>
	</div>

	<div class="row margin-v-10">
		<div class="col">
			<div class="input-wrap">
				<div class="input-label">身份证号</div>
				<div class="input-holder">
					<input type="tel" formatType="idCard" placeholder="请输入身份证">
				</div>
				<div class="input-del">
					<i class="icon-delet" role="button"></i>
				</div>
			</div>
		</div>
	</div>

	
	<p class="demo-p">3 没有标题 </p>
	<div class="row margin-v-10">
		<div class="col">
			<div class="input-wrap">
				<div class="input-holder">
					<input type="text" placeholder="输入姓名" />
				</div>
				<div class="input-del">
					<i class="icon-delet" role="button"></i>
				</div>
				<div class="input-icon">
					<i class="icon-card" role="button"></i>
				</div>
			</div>
		</div>
	</div>


## hercules polyfill 


### 效果如下

[hercules.polyfill.js](http://wikieswan.github.io/vera3/example/index.html#herculespolyfill)

<iframe src="http://wikieswan.github.io/vera3/example/index.html#herculespolyfill"
 style="width:375px; height: 667px;border: 2px solid #ddd;border-bottom: 50px solid #6D6767;border-top: 50px solid #6D6767;border-radius: 10px;"></iframe>


[更多信息](http://vera.ren/hercules-polyfill)



## 表单 input


### 效果如下

[表单 input](http://wikieswan.github.io/vera3/example/index.html#input)

<iframe src="http://wikieswan.github.io/vera3/example/index.html#input"
 style="width:375px; height: 667px;border: 2px solid #ddd;border-bottom: 50px solid #6D6767;border-top: 50px solid #6D6767;border-radius: 10px;"></iframe>

### 用法
	
	//1 标准表单 
	<div class="input-wrap">
		<div class="input-label">输入标题</div>
		<div class="input-holder">
			<input type="text" placeholder="输入银行卡号" />
		</div>
		<div class="input-del">
			<i class="icon-delet"></i>
		</div>
	</div>

	//2 ICON
	<div class="input-wrap">
		<div class="input-label">输入标题</div>
		<div class="input-holder">
			<input type="text" placeholder="输入银行卡号" />
		</div>
		<div class="input-del">
			<i class="icon-delet"></i>
		</div>
		<div class="input-icon">
			<i class="icon-card"></i>
		</div>
	</div>

	//3 没有标题
	<div class="input-wrap">
		<div class="input-holder">
			<input type="text" placeholder="输入银行卡号" />
		</div>
		<div class="input-del">
			<i class="icon-delet"></i>
		</div>
		<div class="input-icon">
			<i class="icon-card"></i>
		</div>
	</div>

	//4 没有ICON 
	<div class="input-wrap">
		<div class="input-label">输入标题</div>
		<div class="input-holder">
			<input type="text" placeholder="输入银行卡号" />
		</div>
		<div class="input-del">
			<i class="icon-delet"></i>
		</div>
	</div>

	//5 没有ICON、没有标题 
	<div class="input-wrap">
		<div class="input-holder">
			<input type="text" placeholder="输入银行卡号" />
		</div>
		<div class="input-del">
			<i class="icon-delet"></i>
		</div>
	</div>


## 对话框 dialog


### 效果如下

[dialog](http://wikieswan.github.io/vera3/example/index.html#dialog)

<iframe src="http://wikieswan.github.io/vera3/example/index.html#dialog"
 style="width:375px; height: 667px;border: 2px solid #ddd;border-bottom: 50px solid #6D6767;border-top: 50px solid #6D6767;border-radius: 10px;"></iframe>

### api

1 showDialog

	//显示一个普通的 dialog,且内容是HTML格式
	var param = {
		isHTML:true,
		title:'标题',
		content:'<strong>内容</strong>这是内容',
        buttonLeftText: "取消",
        buttonLeftEventMethod: function(){
        	vera.hideDialog()
        	console.log('cancel')
        },
        buttonRightText: "确定",
        buttonRightEventMethod: function(){
        	vera.hideDialog()
        	console.log('ok')
        },
	}
    vera.showDialog(param)

    //显示一个底部有一个按钮的 dialog 
	var param = {
		title:'标题',
		content:'这是内容',
        buttonCenterText: "确定",
        buttonCenterEventMethod: function(){
        	vera.hideDialog()
        	console.log('cancel')
        },
	}
    vera.showDialog(param)

    //显示无标题的 dialog 
	var param = {
		content:'这是内容',
        buttonCenterText: "确定",
        buttonCenterEventMethod: function(){
        	vera.hideDialog()
        	console.log('cancel')
        },
	}
    vera.showDialog(param)

2 hideDialog
	
	// 隐藏dialog
	vera.hideDialog()


## 半屏浮层 halfModal


### 效果如下

[halfModal](http://wikieswan.github.io/vera3/example/index.html#halfModal)

<iframe src="http://wikieswan.github.io/vera3/example/index.html#halfModal"
 style="width:375px; height: 667px;border: 2px solid #ddd;border-bottom: 50px solid #6D6767;border-top: 50px solid #6D6767;border-radius: 10px;"></iframe>

### api

1 showHalfModal

	//显示一个普通的 halfModal,且内容是HTML格式
	var param = {
		title:'<div>标题 <span style="color:red">副标题</span></div>',
		subTitle:'子标题',
		content:'<strong>内容</strong>这是容内',
        buttonText: "取消",
	    buttonEventMethod: function(){
	    	vera.hideHalfModal()
	    },
	    closeEventMethod:function(){
	    	vera.hideHalfModal()
	    },
	    forgetEventMethod:function(){
	    	vera.hideHalfModal()
	    },
	}
    vera.showHalfModal(param)

    //显示无子标题和问号的 halfModal 
	var param = {
		title:'标题',
		content:'这是内容内容内容内容',
        buttonText: "取消",
	    buttonEventMethod: function(){
	    	vera.hideHalfModal()
	    }
	}
    vera.showDialog(param)

    //显示无按钮的 halfModal 
	var param = {
		title:'标题',
		subTitle:'标题',
		content:'这是内容内容内容内容内容内容内容内容内',
        buttonText: "取消",
	    closeEventMethod:function(){
	    	vera.hideHalfModal()
	    },
	    forgetEventMethod:function(){
	    	vera.hideHalfModal()
	    },
	}
    vera.showDialog(param)

2 hideHalfModal
	
	// 隐藏HalfModal
	vera.hideHalfModal()








