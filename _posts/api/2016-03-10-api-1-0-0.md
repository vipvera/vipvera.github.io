...
vera 依赖 zepto.js 已经 zepto.js 的 touch 模块（我们使用 ```tap``` 事件），就像 bootstrap 依赖 jquery 一样，在引入 vera.js前，请务必先引入 jquery 。
...
它的按钮默认是唯品红色，但是现在的需求是，获取验证码的按钮是蓝色的，那么极致的我们就在 chrome 浏览器下查看按钮的样式定义，然后重写它，如下：
