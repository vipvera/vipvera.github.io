...
```.btn``` 的宽度是父级宽度的100%；可以通过控制父级的宽度来设置btn的宽度。

### 按钮组件 - buttonLoading

加载按钮 buttonLoading 是针对用户和后台交互的一个友好的组件，当用户点击按钮，向服务器发送了一个借口请求，一般来说就是调用了一次 ajax ，这时候给按钮设置为 loading 状态，防止用户在 ajax 返回结果前用户一直点击按钮发送请求。
html的代码如下：
